...
const ItemForm = {
  render(kind, item) {
    ...
    Form.render(newView, (edit ? 'Edit ' : 'Add ') + kind, this.fields(kind, item), buttons, back);
  ...
  fields(kind, item) {
    ...
    if (kind === 'task') {
      return (
        ...
        Form.field('Branch', Form.input('f-branch', v('branch', currentBranch), { list: 'branchList' })) +
        ...
        Form.field('Notes', Form.textarea('f-notes', v('notes')))
      );
    }
    return (
      Form.field('Title', Form.input('f-name', v('name'), { placeholder: 'Title' })) +
  ...
  readFields(kind) {
    ...
    return { name, start: Form.read('f-start'), url: Form.read('f-url').trim() || null, notes: Form.read('f-notes') || null };
  },
